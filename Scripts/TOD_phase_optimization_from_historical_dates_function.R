TOD_phase_optimization_from_historical_dates <- function(site,dates_list,analysis_time_type){
  library(dplyr)
  if(substr(site,1,3) == "100" & nchar(site)==5){
    Rt_code <- "Rt1"
  }
  if(substr(site,1,3) == "130" & nchar(site)==5){
    Rt_code <- "Rt130"
  }
  if(substr(site,1,2) == "73" & nchar(site)==4){
    Rt_code <- "Rt73"
  }
  if(substr(site,1,2) == "18" & nchar(site)==4){
    Rt_code <- "Rt18"
  }
  ...
  for (p_k in dates_list){
    ...
    df2 <- sqldf::sqldf("select a.phases, a.rank, b.`Duration (s)`, b.Start, b.End, b.Gapped, c.date, b.start_time, c.day, c.start_time2, c.actual_cycle_length from df1 as a left join df as b on a.rank = b.rank and a.phases = b.Phase left join rank_df as c on a.rank = c.rank", drv = "SQLite")
    ...
    df4 <- sqldf::sqldf("select b.time, a.start_time3, b.phases, a.day, a.date, a.split_percentage, a.actual_cycle_length from time_df2 as b left join df3 as a on b.time = a.start_time3 and b.phases = a.phases", drv = "SQLite")
  ...
  df_final_selected <- sqldf::sqldf("select a.time, a.phases, avg(a.split_percentage) as split_percentage, avg(a.actual_cycle_length) as avg_cycle_length from df_final_selected as a group by a.time, a.phases", drv = "SQLite")
