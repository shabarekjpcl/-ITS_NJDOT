...
df222 <- sqldf::sqldf("select b.subsite, b.Date, b.Day, a.time, b.Cycle_Length  from df_time as a
left join df22 as b
...
df222$zone <- as.integer(readr::read_csv(zones_file)[readr::read_csv(zones_file)$Site==df222$subsite[1],]$Zone)
...
df222_final2 <- sqldf::sqldf("select a.time, a.zone, a.weektype, avg(a.Cycle_Length) as Cycle_Length
                             from df222_final as a
                             where a.weektype <> 'Monday' and a.weektype <> 'Friday'
                             group by a.time, a.zone, a.weektype")
df222_final2$time <- hms::as_hms(df222_final2$time)
return(df222_final2)
}
